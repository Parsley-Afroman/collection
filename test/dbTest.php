...
class dbTest extends TestCase {
    public function testSuccessLoop(){
        ...
        $inputA = $array;
        $case = jacketMarkup($inputA);
        $this->assertEquals($expected, $case);
    }
    public function testFailureLoop(){
        $array = [['name' => '', 'designer' => '', 'style' => '', 'year_released' => 2035, 'image' => '']];
        $expected = '<div class="item"><div class = "empty">there is no image to present at this time</div><div class = "empty">there isn\'t a name for this item</div><div class = "name"></div><div class = "empty">the designer of this garment is not yet known</div><div class="designer"></div><div class = "empty">the style of this garment is not yet known</div><div class = "style"></div><div class = "empty">select a year that isn\'t in the future</div>';
        $inputA = $array;
        $case = jacketMarkup($inputA);
        $this->assertEquals($expected, $case);
    }
